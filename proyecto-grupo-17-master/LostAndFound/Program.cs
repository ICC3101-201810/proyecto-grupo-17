...
namespace LostAndFound
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Ubicacion ubicacion7 = new Ubicacion("Cancha inferior   ","cancha niver san carlos");
            ...
            while (keeplooping)
            {
                ...
                {
                    Console.WriteLine("La opcion ingresada no valida");
                    continue;
                }
            }
            Console.WriteLine("\nBienvenido al Lost And Found UAndes {0}       ", usuarios_no_iguales[usuarios_no_iguales.Count() -1].nombre_usuario  );
            while (opcion != "8")
            {
                Console.WriteLine(" \nIngrese una opcion : \n" +
                ...
                if (opcion == "2")
                {
                    Console.WriteLine("CODIGO            NOMBRE OBJETO                  UBICACION                      INDICACION");
                    foreach (Objeto objeto in objeto_perdido ) 
                    {
                        Console.WriteLine(objeto.codigo+"                 " + objeto.descripcion + "                 " +objeto.ubicacion.Nombre_lugar+ "             "+ objeto.ubicacion.indicacion);
                    }
